...
function parse_station($str)
{
	...
	$station = $res['name']."駅&nbsp;".getParam($val,'walk');
...
function hit_feature($param)
{
	...
	$res = array();
	if(in_array(1,getParam($param,'treatment')) == true){$res[] = '賞与';}
	...
	else if(in_array(array(6,7,8,9),getParam($param,'treatment'))){
		if(in_array(6,getParam($param,'treatment')) == true){$res[] = '厚生年金';}
		if(in_array(7,getParam($param,'treatment')) == true){$res[] = '雇用保険';}
		if(in_array(8,getParam($param,'treatment')) == true){$res[] = '健康保険';}
		if(in_array(9,getParam($param,'treatment')) == true){$res[] = '労災';}
	}
...
function getSalaryData($param =array())
{
	...
	if(count($param) > 0)
	{
		foreach($param as $salary)
		{
			...
			//給与
			$salary_txt = "<strong>".$salary['job_category']."</strong>"."【".$salary['salary_type']."】".escapeHtml($salary['salary_price']).$salary['salary_unit'];
 ...
 *
 */
function create_favorite_btn($salon_id,$account=NULL)
{
	...
	if($account==NULL)
	{
		return '<a href="/mypage/login?back=/salon/'.$salon_id.'?fv_add=true" data-id="'.$salon_id.'"><img src="/common_pc/img/salon/btn_favorite.png" /></a>&nbsp;';
	}
	...
	foreach($salons as $favorite_id)
	{
		...
		{
			return '<a href="javascript:void(0);"  class="rem_fav_btn" data-id="'.$salon_id.'"><img src="/common_pc/img/salon/btn_favorite_off.png" /></a>&nbsp;';
		}
	}
	
	return '<a href="javascript:void(0);"  class="add_fav_btn" data-id="'.$salon_id.'"><img src="/common_pc/img/salon/btn_favorite.png" /></a>&nbsp;';
}
