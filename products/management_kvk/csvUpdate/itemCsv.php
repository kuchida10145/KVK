<?php
	...
	// ボタン表示判定
	if($resultMessage == "成功<br>") {
		$btn = '<button type="submit" class="btn btn-success"  onclick="document.form.submit();" name="run_button" value="run">CSV 取込</button>';
		$disabled		= "";
		$disabled_2		= "disabled";
	} else {
		$btn = '<button type="submit" class="btn btn-default" onclick="document.form.submit();" name="test_button" value="test">取込テスト</button>';
		$disabled		= "disabled";
		$disabled_2		= "";
	}
?>
					...
					<input type="hidden" name="mode" value="step1" />
						<div class="form-group">
							...
							<div class="col-sm-10">
								...
								<input type="file" class="hidden" id="upload" name="file" value="<?php echo $viewFilePath ?>" <?php echo $disabled_2 ?>>
								<!-- 代わりに input[type=text] と ボタンを表示 -->
								<div class="input-group">
									<input type="text" id="filepath" name="filepath" class="form-control" value="<?php echo $viewFilePath ?>" placeholder="item_master.csv" <?php echo $disabled_2 ?>>
									<span class="input-group-btn">
										<!-- ボタンを押したときに input[type=file] を押したことにする -->
										<a class="btn btn-default" onclick="document.getElementById('upload').click()" <?php echo $disabled_2 ?>>ファイル選択</a>
									</span>
								</div>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<div class="btn-group">
									<div class="dropdown">
										<button class="btn btn-default dropdown-toggle" type="button" id="pdfDay" name="pdfDay" data-toggle="dropdown" onClick="changeDay(this.form)" value="<?php echo $pdfDay ?>" <?php echo $disabled ?>>
											<?php echo $pdfDay ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<div class="input-group">
									<input style="width:50px;" type="text" id="hour" name="hour" class="form-control"  maxlength="2" pattern="^[0-9]+$" value="<?php echo $pdfHour ?>" <?php echo $disabled ?>>
									<label for="upload" class="col-sm-2 control-label">:</label>
									<input style="width:50px;" type="text" id="min" name="min" class="form-control"  maxlength="2" value="<?php echo $pdfMin ?>" <?php echo $disabled ?>>
								</div>
							</div>
						</div>
						<div align="center">
